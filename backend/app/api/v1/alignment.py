...
from app.services.alignment_auto_realign import auto_local_realign
...
class AutoRealignRequest(BaseModel):
    created_by: str | None = None


class AutoRealignResponse(BaseModel):
    revision: AlignmentRevision
    diagnostics: dict = Field(default_factory=dict)
...
@router.post(
    "/tracks/{track_id}/alignment/auto-realign",
    response_model=AutoRealignResponse,
    summary="Create an automatic local drift-recovery draft revision",
)
async def auto_realign_track_alignment(
    track_id: str,
    payload: AutoRealignRequest,
    x_admin_secret: str | None = Header(default=None),
    repo: PgRepository = Depends(get_repo),
) -> AutoRealignResponse:
    _require_admin_secret(x_admin_secret)
    ...
    active = await _ensure_initial_revision(track_id, repo)
    if active is None:
        raise HTTPException(
            status_code=404,
            detail="Active alignment revision not found.",
        )

    source_document = active.document or syllable_timings_to_document(
        active.syllable_timings
    )
    result = auto_local_realign(
        source_document,
        lyrics_source=track.lyrics_source,
    )
    ...
    revision = AlignmentRevision(
        track_id=track_id,
        revision_no=await repo.next_alignment_revision_no(track_id),
        source="auto_global_dp",
        lyrics_text=lyrics_text_from_document(result.document),
        syllable_timings=document_to_syllable_timings(result.document),
        document=result.document,
        operations=[],
        diagnostics=result.diagnostics,
        is_published=False,
        created_by=payload.created_by or "system:auto-realign",
    )
    stored = await repo.create_alignment_revision(revision)
    return AutoRealignResponse(revision=stored, diagnostics=result.diagnostics)

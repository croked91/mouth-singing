...
interface AlignmentEditorProps {
  ...
  onAutoRealign: () => Promise<AlignmentRevision>;
}
...
export const AlignmentEditor: React.FC<AlignmentEditorProps> = ({
  ...
  const runAutoRealign = async (): Promise<void> => {
    const revision = await onAutoRealign();
    setLastDraft(revision);
    setStatusText(`Авто-черновик #${revision.revision_no} создан`);
  };
    ...
    <Box sx={{ minHeight: '100vh', p: 3, color: 'white', background: '#09090f' }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="outlined" disabled={!adminSecret} onClick={runAutoRealign}>Auto realign</Button>

  ...
  return (
    <Box sx={{ background: '#09090f' }}>
      ...
      <AlignmentEditor
        ...
        onAutoRealign={async () => {
          if (!trackId) throw new Error('Track id is missing');
          if (!adminSecret) throw new Error('Введите admin secret');
          const revision = await api.autoRealignTrack(trackId, adminSecret, 'admin:auto');
          const refreshed = await api.getTrackAlignment(trackId);

...
# ---------------------------------------------------------------------------
# 1. Full E2E scenario — complete user journey
# ---------------------------------------------------------------------------


class TestFullE2EJourney:
    """Walk through the entire user journey from session creation to termination."""

    async def test_complete_karaoke_session_lifecycle(self, client, app_db):
        """
        Full journey:
        1. Create session
        ...
        create_resp = await client.post(
            "/api/v1/sessions", json={"room_id": "e2e-journey-room"}
        )
        assert create_resp.status_code == 201
        session = create_resp.json()
        session_id = session["id"]
        assert session["room_id"] == "e2e-journey-room"
        assert session["status"] == "active"
        assert session["terminated_at"] is None

        # ------------------------------------------------------------------
        # Step 2: Add two participants
        # ------------------------------------------------------------------
        ...
        participant1_id = p1_resp.json()["id"]
        assert p1_resp.json()["display_name"] == "Мария"
        ...
        participant2_id = p2_resp.json()["id"]
        assert p2_resp.json()["display_name"] == "Дмитрий"

        # Verify GET session returns both participants
        session_detail = await client.get(f"/api/v1/sessions/{session_id}")
        assert session_detail.status_code == 200
        participants = session_detail.json()["participants"]
        assert len(participants) == 2
        participant_names = {p["display_name"] for p in participants}
        assert participant_names == {"Мария", "Дмитрий"}

        # ------------------------------------------------------------------
        # Step 3: Seed the catalog and search for tracks
        # ------------------------------------------------------------------
        track1_id = await _create_track(
            repo, artist="Кино", title="Группа крови", clip_path="/media/kino.mp4"
        )
        track2_id = await _create_track(
            repo, artist="Агата Кристи", title="Вечная любовь", clip_path="/media/agata.mp4"
        )

        search_resp = await client.get("/api/v1/tracks/search?q=Кино")
        assert search_resp.status_code == 200
        search_data = search_resp.json()
        assert search_data["total"] >= 1
        found_artists = [item["artist"] for item in search_data["items"]]
        assert "Кино" in found_artists

        # ------------------------------------------------------------------
        # Step 4: Add tracks to queue
        # ------------------------------------------------------------------
        entry1 = await _add_to_queue(client, session_id, participant1_id, track1_id)
        assert entry1["status"] == "queued"
        assert entry1["order_position"] == 1

        entry2 = await _add_to_queue(client, session_id, participant2_id, track2_id)
        assert entry2["status"] == "queued"
        assert entry2["order_position"] == 2

        # ------------------------------------------------------------------
        # Step 5: Get queue state
        # ------------------------------------------------------------------
        queue_resp = await client.get(f"/api/v1/sessions/{session_id}/queue")
        assert queue_resp.status_code == 200
        queue_data = queue_resp.json()
        assert queue_data["current"] is not None
        assert queue_data["current"]["id"] == entry1["id"]
        assert queue_data["current"]["participant"]["display_name"] == "Мария"
        assert queue_data["current"]["track"]["artist"] == "Кино"
        assert len(queue_data["upcoming"]) == 1
        assert queue_data["upcoming"][0]["id"] == entry2["id"]

        # ------------------------------------------------------------------
        # Step 6: Start playing the first entry
        # ------------------------------------------------------------------
        start_resp = await client.post(f"/api/v1/queue/{entry1['id']}/start")
        assert start_resp.status_code == 200
        start_data = start_resp.json()
        assert start_data["entry_id"] == entry1["id"]
        assert start_data["clip_url"] == "/media/kino.mp4"
        assert start_data["duration_sec"] == 210

        # Queue should now show entry1 as "playing"
        queue_after_start = await client.get(f"/api/v1/sessions/{session_id}/queue")
        assert queue_after_start.json()["current"]["status"] == "playing"
        assert queue_after_start.json()["current"]["started_at"] is not None

        # ------------------------------------------------------------------
        # Step 7: Finish playing the first entry
        # ------------------------------------------------------------------
        finish_resp = await client.post(f"/api/v1/queue/{entry1['id']}/finish")
        assert finish_resp.status_code == 200
        finish_data = finish_resp.json()
        assert finish_data["next_entry_id"] == entry2["id"]
        assert finish_data["next_participant"] is not None
        assert finish_data["next_participant"]["display_name"] == "Дмитрий"

        # Verify counters were incremented
        track1 = await repo.get_track(track1_id)
        assert track1.play_count == 1
        p1 = await repo.get_participant(participant1_id)
        assert p1.tracks_played == 1

        # Queue should now only have entry2
        queue_after_finish = await client.get(f"/api/v1/sessions/{session_id}/queue")
        queue_state = queue_after_finish.json()
        assert queue_state["current"]["id"] == entry2["id"]
        assert queue_state["upcoming"] == []

        # ------------------------------------------------------------------
        # Step 8: Skip the second entry
        # ------------------------------------------------------------------
        # Add a third entry so there is something after the skip
        track3_id = await _create_track(
            repo, artist="ДДТ", title="Это всё", clip_path="/media/ddt.mp4"
        )
        entry3 = await _add_to_queue(client, session_id, participant1_id, track3_id)

        skip_resp = await client.post(f"/api/v1/queue/{entry2['id']}/skip")
        assert skip_resp.status_code == 200
        skipped_new_entry = skip_resp.json()
        # New entry preserves the participant and track from the skipped entry
        assert skipped_new_entry["participant_id"] == participant2_id
        assert skipped_new_entry["track_id"] == track2_id
        assert skipped_new_entry["status"] == "queued"
        # New entry is at the end — higher position than entry3
        assert skipped_new_entry["order_position"] > entry3["order_position"]

        # Old entry2 is no longer in the active queue
        queue_after_skip = await client.get(f"/api/v1/sessions/{session_id}/queue")
        active_ids = []
        skip_queue = queue_after_skip.json()
        if skip_queue["current"]:
            active_ids.append(skip_queue["current"]["id"])
        active_ids.extend(u["id"] for u in skip_queue["upcoming"])
        assert entry2["id"] not in active_ids

        # ------------------------------------------------------------------
        # Step 9: Get recommendations (0 tracks played for participant2 → popular)
        # ------------------------------------------------------------------
        rec_resp = await client.get(
            "/api/v1/recommendations",
            params={"participant_id": participant2_id, "session_id": session_id},
        )
        assert rec_resp.status_code == 200
        rec_data = rec_resp.json()
        assert "strategy" in rec_data
        assert "tracks" in rec_data
        # With 0 plays for participant2, should use popular strategy
        assert rec_data["strategy"] == "popular"

        # ------------------------------------------------------------------
        # Step 10: Admin terminate session
        # ------------------------------------------------------------------
        terminate_resp = await client.delete(
            f"/api/v1/sessions/{session_id}",
            headers={"X-Admin-Secret": "test-secret"},
        )
        assert terminate_resp.status_code == 204

        # Verify session is now terminated
        get_terminated = await client.get(f"/api/v1/sessions/{session_id}")
        assert get_terminated.status_code == 200
        terminated_session = get_terminated.json()
        assert terminated_session["status"] == "terminated"
        assert terminated_session["terminated_at"] is not None

...
class TestParticipants:
    ...
    async def test_update_portrait(self, sqlite_repo: SQLiteRepository):
        # Arrange
        session = await sqlite_repo.create_session(_session())
        participant = await sqlite_repo.create_participant(
            _participant(session_id=session.id)
        )
        vector = [float(i) / 100.0 for i in range(45)]

        # Act
        await sqlite_repo.update_portrait(participant.id, vector)
        updated = await sqlite_repo.get_participant(participant.id)

        # Assert
        assert updated is not None
        assert updated.portrait_vector is not None

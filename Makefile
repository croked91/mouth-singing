...
# ---------------------------------------------------------------------------
# Local development (named volumes, requires NVIDIA GPU)
# ---------------------------------------------------------------------------
...
down: ## Stop and remove all containers
	...
	docker compose -f $(BASE) -f $(GPU) down 2>/dev/null; true

down-v: ## Stop, remove containers AND volumes (full reset)
	docker compose -f $(BASE) -f $(GPU) down -v 2>/dev/null; true

logs: ## Tail logs from all services
	docker compose -f $(BASE) -f $(GPU) logs -f --tail=50

logs-worker: ## Tail worker logs only
	docker compose -f $(BASE) -f $(GPU) logs -f --tail=100 worker
...
migrate-qdrant-clusters: ## Backfill rec_cluster_id into QDrant payloads (run BEFORE deploying new backend)
	...
	async def main(): \
	    pool = await asyncpg.create_pool('postgresql://karaoke:karaoke@postgres:5432/karaoke'); \
	    rows = await pool.fetch(\"SELECT id, rec_cluster_id FROM tracks WHERE rec_cluster_id IS NOT NULL AND status = 'ready'\"); \
	    await pool.close(); \
	    by_cluster = defaultdict(list); \
	    [by_cluster[r['rec_cluster_id']].append(r['id']) for r in rows]; \
	    print(f'{len(rows)} tracks, {len(by_cluster)} clusters'); \
	    client = QdrantClient(host='qdrant', port=6333, timeout=300); \
	...
	asyncio.run(main())"
	@echo ""
	@echo "Migration complete. Now rebuild backend: make up-gpu"
